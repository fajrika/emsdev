...
class m_meter_air extends CI_Model
{
    ...
    public function harga_air($data1, $data2, $data3)
    {
        ...
        if ($rumus == 1) {
            ...
            $i = 0;
            do {
                if ($data3 > ($result[$i]['range_akhir'] - ($i != 0 ? $result[$i - 1]['range_akhir'] : 0))) { //selisih { //15 > 10 ? true
                    if ($i == 0) // false
    ...
    public function ajax_save_meter($meter, $periode, $unit_id)
    {
        ...
        $validation = $this->db->from("t_tagihan_air")
            ->where("periode", $periode)
            ->where("unit_id", $unit_id)
            ->get()->row();

        if ($validation->status_tagihan != 1) {
        ...
        return [
            "status" => false,
            "message" => "Tagihan Sudah di Bayar"
        ];
    }
