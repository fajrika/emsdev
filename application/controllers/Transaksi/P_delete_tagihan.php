...
class P_delete_tagihan extends CI_Controller
{
    ...
    public function destroy($unit_id = null)
    {
        if ($this->input->post('method') == 'delete') {
            ...
            // echo ('</pre>');
            $service = $this->db->from('service')->where('id', $this->input->post('service_id'))->get()->row();
            // echo ('<pre>');
            // print_r($service);
            // echo ('</pre>');
            // die;
            if ($service->service_jenis_id == 1) {
                ...
                $this->session->set_flashdata('success', "Data Lingkungan Berhasil di Hapus");
            } elseif ($service->service_jenis_id == 2) {
                $this->db->delete('t_tagihan_air_info', array('t_tagihan_air_id' => $this->input->post('tagihan_id')));
                $this->db->delete('t_tagihan_air_detail', array('t_tagihan_air_id' => $this->input->post('tagihan_id')));
                $this->db->delete('t_tagihan_air', array('id' => $this->input->post('tagihan_id')));
                $this->session->set_flashdata('success', "Data air Berhasil di Hapus");
            }
