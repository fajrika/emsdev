...
class P_master_unit extends CI_Controller
{
    ...
    public function edit()
    {
        ...
        if ($this->input->post('nomor_unit')) {
            ...
            $status = $this->m_unit->edit([
                ...
                'air_aktif' => $this->input->post('meter_air_aktif'),
                // 'tgl_aktif_air' => $this->input->post('tanggal_aktif_air'),
                'tgl_aktif_air' => ($this->input->post('tanggal_aktif_air'))?substr($this->input->post('tanggal_aktif_air'),6,4)."-".substr($this->input->post('tanggal_aktif_air'),3,2)."-".substr($this->input->post('tanggal_aktif_air'),0,2):null,
                'tgl_putus_air' => ($this->input->post('tanggal_putus_air'))?substr($this->input->post('tanggal_putus_air'),6,4)."-".substr($this->input->post('tanggal_putus_air'),3,2)."-".substr($this->input->post('tanggal_putus_air'),0,2):null,
        ...
        if ($this->m_unit->cek($this->input->get('id'))) {
            ...
            $dataPemilik = $this->db->from('customer')->where('id',$dataSelect->pemilik)->get()->row();
            $dataPenghuni = $this->db->from('customer')->where('id',$dataSelect->penghuni)->get()->row();
	...
	{
		$customers = 
			$this->db->select("customer.id, customer.name as text")
			->from('customer')
