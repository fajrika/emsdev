...
class m_desktop_transaksi_lingkungan extends CI_Model
{
    ...
    public function save2($project_id,$source,$denda_jenis_service,$denda_nilai_service,$jarak_periode=0){
        ...
        $tagihan_lingkungan_tmp = $this->db->select("
                                                    ...
                                                    unit.luas_tanah")
                                            ->from("ems_temp.$source.td_lingkungan
                                                    FULL JOIN ems_temp.$source.td_air
                                                    ON td_air.th_trans_id = td_lingkungan.th_trans_id
                                                    AND MONTH(DATEADD(MONTH,$jarak_periode,td_air.periode)) = MONTH(DATEADD(MONTH,$jarak_periode,td_lingkungan.periode))
                                                    AND YEAR(DATEADD(MONTH,$jarak_periode,td_air.periode)) = YEAR(DATEADD(MONTH,$jarak_periode,td_lingkungan.periode))" 
                                                    )
                                            ->join("ems_temp.$source.th_trans",
                                                    "th_trans.th_trans_id = td_lingkungan.th_trans_id
                                            ...
                                            ->join("t_tagihan_lingkungan",
                                                    ...
                                                    AND 
                                                        (
                                                            (
                                                                MONTH(t_tagihan_lingkungan.periode) = MONTH(DATEADD(MONTH,$jarak_periode,td_air.periode)) 
                                                                AND YEAR(t_tagihan_lingkungan.periode) = YEAR(DATEADD(MONTH,$jarak_periode,td_air.periode))
                                                            )
                                                        OR  
                                                            (
                                                                MONTH(t_tagihan_lingkungan.periode) = MONTH(DATEADD(MONTH,$jarak_periode,td_lingkungan.periode)) 
                                                                AND YEAR(t_tagihan_lingkungan.periode) = YEAR(DATEADD(MONTH,$jarak_periode,td_lingkungan.periode))
                                                            )
                                                        )",
                                                    "LEFT")
                                            ...
                                            // ->where_in("")
                                            ->where("td_lingkungan.id is null")
                                            // ->order_by("td_lingkungan.id asc")
        ...
        foreach ($tagihan_lingkungan_tmp as $k=>$v) {
            ...
            if($double == 0){
                ...
                $this->db->where('unit_id',$tagihan_lingkungan->unit_id);
                $this->db->where('periode',$tagihan_lingkungan->periode);
                ...
                if (!$tagihan_sudah_ada->num_rows()) {
                    ...
                    $tagihan_lingkungan->t_tagihan_id = $tagihan_sudah_ada->row()->id;
                }
                $i++;
    ...
    public function save_air($project_id,$source,$denda_jenis_service,$denda_nilai_service,$jarak_periode=0){
        ...
        $tagihan_lingkungan_tmp = $this->db->select("
                                            ...
                                            //         "t_tagihan_lingkungan.proyek_id = '$project_id'
                                            //         AND t_tagihan_lingkungan.unit_id = unit.id
                                            //         AND (MONTH(t_tagihan_lingkungan.periode) = MONTH(DATEADD(MONTH,$jarak_periode,td_air.periode)) 
                                            //             AND YEAR(t_tagihan_lingkungan.periode) = YEAR(DATEADD(MONTH,$jarak_periode,td_air.periode)))",
                                            //         "LEFT")
                                            // ->where("td_air.periode is not null")
                                            // ->where_in("")
                                            ->where("t_tagihan_lingkungan_detail.t_tagihan_lingkungan_id is null")
